...
class Destalinator(WithLogger, WithConfig):
    ...
    def stale(self, channel_name, days):
		...
		for x in messages
			if x.get("user") in self.config.ignore_users
				self.logger.debug("Stale: User %s ignored in %s", x.get("user"),channel_name)
			if x.get("username") in self.config.ignore_users
				self.logger.debug("Stale: Username %s ignored in %s", x.get("username"),channel_name)
    ...
    def archive(self, channel_name):
        ...
        if self.config.activated:
            ...
            self.action(u"Archiving channel #{}".format(channel_name))
    ...
    def warn(self, channel_name, days, force_warn=False):
        ...
        if self.config.activated:
            ...
            self.action(u"Warned #{}".format(channel_name))
